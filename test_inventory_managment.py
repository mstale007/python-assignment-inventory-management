...
class TestInventory(unittest.TestCase):
    ...
    def setUp(self):
        self.test_inventory=Inventory(load_from_backup=True)
    ...
    def test_search_by_id(self):
        ...
        self.assertTrue(self.test_inventory.search_by_id('A1'))
    
    def test_search_by_id_notexists(self):
        """Test for Inventory search by ID, not exists"""
        self.assertFalse(self.test_inventory.search_by_id('B1'))

    def test_add_item(self):
        """Test for Inventory add item"""
        self.test_inventory.add_item(Product("A10","TestName",300,1))
        self.assertIsNotNone(self.test_inventory.search_by_id("A10"))

    def test_update_item(self):
        """Test for Inventory update item"""
        self.test_inventory.update_item(Product("A1","iPhone 18",300,1))
        result=self.test_inventory.search_by_id("A1")
        self.assertEqual(result.name,"iPhone 18")
    
    def test_delete_item(self):
        """Test for Inventory delete item"""
        self.assertTrue(self.test_inventory.delete_item("A6"))
